...
if [ ! -f "/tmp/.a1_sys_pkg_checked" ]; then
    ...
    # 최신 노드(WanVideo) 호환을 위해 Torch 2.5.1 + CUDA 12.4 조합으로 업그레이드
    # (torch 2.4.1 + cuDNN 9.x 조합은 CUDNN_STATUS_NOT_INITIALIZED 에러 발생)
    pip install torch==2.5.1 torchvision==0.20.1 torchaudio==2.5.1 --index-url https://download.pytorch.org/whl/cu124 || echo '⚠️ Torch 재설치 실패'
    ...
    pip install --no-cache-dir \
        GitPython onnx onnxruntime-gpu==1.18.0 opencv-python-headless tqdm requests \
    ...
    # [중요] numpy 2.x 충돌 방지 - onnxruntime-gpu 등이 numpy 2.x를 당기므로 1.x로 고정
    pip install numpy==1.26.4 --force-reinstall || echo '⚠️ numpy 고정 실패'
    ...
    # xformers 버전 고정 (미지정 시 torch 2.6.0으로 강제 업그레이드되는 문제 방지)
    pip install xformers==0.0.29.post1 --index-url https://download.pytorch.org/whl/cu124 || echo '⚠️ xformers 설치 실패'
